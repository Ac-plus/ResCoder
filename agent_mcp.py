...
def _mcp_server_main() -> None:
    """
    MCP server 子进程入口：通过 FastMCP 暴露 tools。
    重点：
    - stdio transport 的 stdout 是协议通道，不能被 print 污染
    - 所以把工具函数内部 stdout 重定向到 stderr
    """
    ...
    # ✅ 注意：tool name 必须与 LLM tools schema 的 name 完全一致
...
# ======================================================================
# ✅ MCP CLIENT（同步版）：Agent 用它来 tools/call
# ======================================================================
    ...
    def run(self, user_input: str, max_rounds: int = 30, confirm_terminal: bool = True) -> str:
        ...
        for round_idx in range(1, max_rounds + 1):
            ...
            for tc in tool_calls:
                ...
                # ✅ 对 run_terminal_command 做确认（并防止“非交互 stdin”卡死）
...
if __name__ == "__main__":
    # --mcp-server 表示“以 MCP server 模式启动”（只跑 tools 服务，不跑 agent）
    # 说明：--mcp-server 是长选项，用于切换模式（不带短选项）。
