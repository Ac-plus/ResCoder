...
# ✅ 改为：从命令行指定“引用源”后，再从 py 文件里取变量（不在命令行输入提示词文本）
...
# ======================================================================
# ✅ MCP Streamable HTTP Client（同步封装）：按 1.25.0 官方返回 3 元组
# ======================================================================
class MCPStreamableHTTPClient:
    ...
    async def _with_session(self):
        """
        官方示例：streamable_http_client 会 yield (read_stream, write_stream, _)
        这里写成一个小工具，避免你再踩 “expected 2 got 3”。
        """
...
# ======================================================================
# ✅ Function Calling Agent（工具执行层改为 MCP HTTP）
# ======================================================================
class FunctionCallingAgent:
    ...
    def run(self, user_input: str, max_rounds: int = 30, confirm_terminal: bool = True) -> str:
        ...
        # ]
        # ===== RAG retrieve (FAQ + Standards each 3 chunks) =====
        ########
        try:
            rag = rag_retrieve(
                query=user_input,
