...
# ----------------------
# 1) 小工具：缓存（内存版） - TinyLFU (simplified W-TinyLFU)
# ----------------------

class _CountMinSketch(object):
    """
    Count-Min Sketch with periodic aging (halving).
    Used by TinyLFU to estimate access frequency approximately.
...
class _LRUWithTTL(object):
    """
    Minimal LRU with TTL based on OrderedDict semantics.
    Implemented without importing OrderedDict at module top to keep changes localized.
    """
...
class _TinyLFU(object):
    """
    Simplified W-TinyLFU for STRICT key matching (payload must be identical).

    Structure:
      - Window LRU: absorbs new keys (recency)
      - Main LRU: stores admitted keys (recency, but admission is frequency-based)
      - Count-Min Sketch: approximate frequency
      - Doorkeeper: simple set (seen-once filter) to block one-hit wonders

    Admission:
      - When window overflows, evict candidate from window.
      - If candidate is first-seen => record in doorkeeper and drop.
      - Else, compare freq(candidate) vs freq(victim=main LRU).
        Admit if freq(candidate) > freq(victim), replacing victim.
    """
...
class SimpleTTLCache(object):
    ...
    Keeps the same constructor signature and get/set interface as before,
    but internally uses simplified TinyLFU eviction/admission.

    NOTE: strict cache hit condition is unchanged because key is identical string.
    """
...
# ----------------------
# 3) Bocha Web Search 后端（国内可用）
# ----------------------

class BochaBackend(SearchBackend):
    """
    Bocha Web Search API（官方）
    - POST https://api.bochaai.com/v1/web-search
    - Header: Authorization: Bearer BOCHA-API-KEY
    - Body: {query, summary, freshness, count}
    参考官方产品文档：Web Search API / freshness / count 取值等。
    """

    def __init__(self):
        # 推荐放环境变量：BOCHA_API_KEY
        self.api_key = BOCHA_API_KEY # os.getenv("BOCHA_API_KEY") or os.getenv("BOCHA_SEARCH_API_KEY")
        # 官方示例入口：/v1/web-search
    ...
    def _recency_to_freshness(recency_days: Optional[int]) -> str:
        """
        Bocha freshness 可选：oneDay/oneWeek/oneMonth/oneYear/noLimit
        这里用 recency_days 做一个合理映射。
        """
    ...
    def _extract_items_from_response(data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        官方文档提到 Response 格式兼容 Bing Search API，
        所以优先尝试从 webPages.value 提取；如果不符合，则做兜底解析。
        """
        # 兼容 Bing：{"webPages": {"value": [ ... ]}}
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                for idx, it in enumerate(raw_items[:top_k]):
                    # Bocha Web Search 文档：网页包括 name/url/snippet/summary/siteName/siteIcon/publishedTime 等
                    results.append({
                        "id": "r{0}".format(idx + 1),
...
def _get_backend() -> SearchBackend:
    """
    通过环境变量 SEARCH_BACKEND 选择后端:
    - "bocha"（默认）
    - 你也可以继续保留 "serpapi"/"bing" 等其它实现
    """
...
def web_search(
    ...
    """
    Function Calling Tool: web_search

    输入:
      - query: 搜索关键词
      - top_k: 返回结果数（建议 5~10）
      - recency_days: 近 N 天优先（可选）
      - domains: 限定域名列表（可选），如 ["openai.com", "github.com"]

    输出:
      - JSON 字符串：{ok, query, results:[{id,title,url,snippet,source,published_time,...}]}
    """
    payload = {
        "query": query,
        "top_k": int(top_k),
